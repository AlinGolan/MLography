...
# Import only the paper postprocessing method
from postprocess import paper_postprocess
...
def evaluate_models(model_list, images, labels, is_gb=True):
    ...
    for name, path in model_list:
        if not os.path.exists(path):
            if is_gb:
                records.append({"Category": "Grain Boundaries", "Model Name": name, "ROC-AUC": "N/A", "IoU (Paper)": "N/A", "OIoU": "N/A"})
            else:
                records.append({"Category": "Impurities", "Model Name": name, "ROC-AUC": "N/A", "IoU (Paper)": "N/A", "OIoU": "N/A"})
            continue
        ...
        try:
            ...
            if np.isnan(raw_preds).any():
                if is_gb:
                    records.append({"Category": "Grain Boundaries", "Model Name": name, "ROC-AUC": "Div", "IoU (Paper)": "Div", "OIoU": "Div"})
                else:
                    records.append({"Category": "Impurities", "Model Name": name, "ROC-AUC": "Div", "IoU (Paper)": "Div", "OIoU": "Div"})
                continue
            ...
            if is_gb:
                ...
                print(f"Done: {name} -> ROC-AUC: {auc:.4f} | IoU (Paper): {mean_iou_paper:.4f}")
                ...
                for i in range(len(labels)):
                    ...
                    # Impurities: Probability binarization (threshold > 0.5) with small noise suppression
            ...
            if is_gb:
                records.append({"Category": "Grain Boundaries", "Model Name": name, "ROC-AUC": "Err", "IoU (Paper)": "Err", "OIoU": "Err"})
            else:
                records.append({"Category": "Impurities", "Model Name": name, "ROC-AUC": "Err", "IoU (Paper)": "Err", "OIoU": "Err"})

    return records

# 1. Models setup (Restricted to Early Stopping models)
...
# 2. Evaluation
print("Evaluating GB Early Stopping model...")
...
print("\nEvaluating Impurities Early Stopping model...")
...
# 3. Render Table with adjusted compact dimensions
df = pd.DataFrame(results)
fig, ax = plt.subplots(figsize=(7.5, len(df) * 0.45 + 1.0))
ax.axis('off')
ax.axis('tight')

table = ax.table(
    cellText=df[["Category", "Model Name", "ROC-AUC", "IoU (Paper)", "OIoU"]].values,
    colLabels=["Category", "Model Name", "ROC-AUC", "IoU (Paper)", "OIoU"],
    loc='center',
    cellLoc='center'
)
...
# Set custom proportional column widths to keep it narrow and neat
col_widths = [0.22, 0.31, 0.15, 0.17, 0.15]
...
plt.title("Early Stopping Model Benchmark Performance", fontsize=12, weight="bold", pad=12)
out_file = "visuals/metrics_tables/models_benchmark_early_stopping.png"
